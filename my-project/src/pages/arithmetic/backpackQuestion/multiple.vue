<template>
  <div class="backpack-multiple-wrapper">
    <h1 class="title">多重背包问题</h1>
    <p class="desc">
      有n件物品和1个容量为W的背包。<span class="important">每种物品最多有numbers[i]件可用</span>，第i件物品的重量为weights[i]，
  ...
  data () {
    return {
      ...
      goods: [ // 商品
        ...
        {count: 2, val: 4, cost: 2},
        {count: 4, val: 5, cost: 7},
        {count: 1, val: 7, cost: 3},
        {count: 2, val: 2, cost: 1},

        // {count: 2, val: 1, cost: 1},
        // {count: 2, val: 8, cost: 4},
        // {count: 1, val: 6, cost: 3},
        // {count: 3, val: 9, cost: 6},
        // {count: 4, val: 10, cost: 7},
      ],
      aSelected: [],
      nTotalCost: 0,
      nTotalVal: 0
  ...
  methods: {
    ...
    knapsack: function () {
      ...
      for (let i=0; i<NGOODS; i++) {
        ...
        let nMaxCount = this.goods[i].count;
        ...
        for (let j=0; j<=NCAPACITY; j++) {
          ...
          if (j < nCost) {
            ...
            aResultMatrix[i][j] = 0;
            let nBound = Math.min(j / nCost, nMaxCount);
    ...
    myKnapscak: function () {
      ...
      for (let i=0; i<nLen; i++){
        ...
        if (nRemainSpace >= oCurrGood.cost) {
          // 还有剩余空间，放入尽量多的当前物品
          let maxCount = Math.min(Number.parseInt(nRemainSpace / oCurrGood.cost), oCurrGood.count);
          ...
          for (let j=aSelected.length-1; j>=0; j--) {
            ...
            const nMaxK = Math.min(Math.floor((oTarget.cost * oTarget.count + nRemainSpace) / oCurrGood.cost), oCurrGood.count);
            // 从一个开始放
